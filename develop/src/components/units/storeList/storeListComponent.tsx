...
import { BusinessInfo } from '../../../common/stores/type';
...
export default function StoreListComponent(): JSX.Element {
    ...
    const [data, setData] = useState<any[]>([]); // data를 배열로 설정
    ...
    useEffect(() => {
        ...
        script.onload = async () => {
            ...
            try {
                ...
                const stores = result.RegionMnyFacltStus[1].row.filter((store: any) =>
                    store.SIGUN_NM === region && store.LEAD_TAX_MAN_STATE_CD !== "03" 
                ); // 폐업한 store제외
